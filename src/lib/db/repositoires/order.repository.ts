...
import { Roles, Status } from "@common/enums";
...
export class OrderRepository {
    ...
    async create(userId: string, createOrderDto: CreateOrderDto, tr_id: string): Promise<OrderDocument> {
    ...
    async findAll(userId: string, data: SearchOrderDto) {
        ...
        const [orders, total] = await Promise.all([
            this.OrderModel.aggregate([{ $match: matchFilter }, { $sort: { createdAt: -1 } }, { $skip: skip }, { $limit: limit }]),
